...
import {
    ...
    CheckCircle2, XCircle, BookOpen, X, Loader2
} from 'lucide-react'
                        ...
                        <div className="flex items-start justify-between border-b border-border p-5">
                            <div className="min-w-0 pr-3">
                                ...
                                <p className="mt-1 text-xs text-muted-foreground">
                                    {done.length}/{students.length} students selesai
                                </p>
                            </div>
                            <button
                                onClick={onClose}
                                ...
                                <>
                                                ...
                                                {done.map((s) => (
                                                    <li key={s.id} className="flex items-center gap-2.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/30 px-3 py-2">
                                                        {s.avatar_url ? (
                                                            ...
                                                            <div className="flex h-7 w-7 items-center justify-center rounded-full bg-emerald-100 dark:bg-emerald-900 text-xs font-bold text-emerald-700">
                                                                {s.name?.charAt(0) ?? '?'}
                                                            </div>
                                                        )}
                                                        <div className="min-w-0 flex-1">
                                                            ...
                                                            <p className="text-xs text-muted-foreground">
                                                                {format(new Date(s.completed_at!), 'MMM d, HH:mm')}
                                                            </p>
                                                        </div>
                                    ...
                                    <div>
                                        <h3 className="mb-2 flex items-center gap-1.5 text-sm font-semibold text-rose-500">
                                            <XCircle size={15} />
                                            Belum mengerjakan ({notDone.length})
                                        </h3>
                                        {notDone.length === 0 ? (
                                                ...
                                                {notDone.map((s) => (
                                                    <li key={s.id} className="flex items-center gap-2.5 rounded-xl bg-rose-50 dark:bg-rose-950/30 px-3 py-2">
                                                        {s.avatar_url ? (
                                                            ...
                                                            <div className="flex h-7 w-7 items-center justify-center rounded-full bg-rose-100 dark:bg-rose-900 text-xs font-bold text-rose-500">
                                                                {s.name?.charAt(0) ?? '?'}
                                                            </div>
                                                        )}
                                                        <p className="truncate text-sm font-medium">{s.name ?? 'Unknown'}</p>
        ...
        <div className="rounded-2xl border border-border bg-card shadow-sm">
            <div className="flex items-center justify-between border-b border-border px-5 py-4">
                <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-sm font-semibold">Mata Kuliah</h2>
            ...
            {addingCustom && (
                <div className="flex items-center gap-2 border-b border-border px-5 py-3">
                    ...
                    />
                    <Button type="button" size="sm" onClick={handleAdd} loading={addCourse.isPending}>Simpan</Button>
                    <Button type="button" size="sm" variant="ghost" onClick={() => { setAddingCustom(false); setCustomInput('') }}>Batal</Button>
                </div>
            )}
                ...
                <div className="divide-y divide-border">
                    {presetCourses.map((c) => (
                        <div key={c.id} className="flex items-center justify-between px-5 py-2.5">
                            ...
                            <span className="rounded-full bg-muted px-2 py-0.5 text-[10px] font-medium text-muted-foreground">preset</span>
                        </div>
                    ))}
                    {customCourses.map((c) => (
                        <div key={c.id} className="flex items-center justify-between px-5 py-2.5">
...
function AdminDashboardContent() {
    ...
    return (
        <>
            {/* Completion drawer (outside scroll context) */}
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                                {tasks.length} task{tasks.length !== 1 ? 's' : ''} in your class
                            </p>
                        </div>
                        <Link
                            id="create-task-btn"
